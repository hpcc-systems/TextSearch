IMPORT XCR_Common;
IMPORT XCR_Storage;

EXPORT Layouts := MODULE
	// Posting Record, work in process.  Inversion records are a subset of the fields
	EXPORT Posting := RECORD
		Types.Nominal							nominal;
		Types.DocID								docID;
		Types.KWP 								kwpBegin;
		Types.KWP 								kwpEnd;
		Types.NodePos							start;
		Types.NodePos 						stop;
		Types.Depth								depth;	
		Types.TagNominal					this;		// this nominal
		Types.TagNominal					parent;	// nominal of parent node
		Types.PathNominal					path;		// cononical path
		Types.TermLength					len;
		Types.WordType						typWord;
		Types.NodeType						typXML;
		Types.Ordinal							preorder;		// position in tree
		Types.Ordinal							parentOrd;	// parent position
		Types.Ordinal							firstOrd;
		Types.Ordinal							lastOrd;
		Types.rci									mcsi;
		Types.rci									pcsi;
		Types.LetterPattern				lp;
		Types.TermString					term{MAXLENGTH(Types.MaxTermLen)};
	END;
	EXPORT ElementPosting := RECORD
		Types.Nominal							nominal;
		Types.DocID								docID;
		Types.KWP 								kwpBegin;
		Types.KWP 								kwpEnd;
		Types.NodePos							nodeStart;
		Types.NodePos 						nodeStop;
		Types.Depth								depth;
		Types.NodePos							firstStart;		// of PCDATA nodes
		Types.NodePos							lastStop;			// of PCDATA nodes
		Types.TagNominal					parent;				// nominal of parent node
		Types.PathNominal					path;					// cononical path
		Types.NodeType						typXML;
		Types.Ordinal							firstOrd;			// first preorder ordinal in subtree
		Types.Ordinal							lastOrd;			// last preoder ordinal in subtree
		Types.Ordinal							parentOrd;		// preorder ordinal of parent
		Types.Ordinal							preorder;
		Types.rci									mcsi;
		Types.rci									pcsi;
	END;
	EXPORT DocPosting := RECORD
		Types.rci									mcsi;
		Types.rci									pcsi;
		Types.DocID								docID;
		Types.KWP 								kwpBegin;
		Types.KWP 								kwpEnd;
		Types.NodePos							nodeStart;
		Types.NodePos 						nodeStop;
		Types.Depth								depthMax;
		Types.NodePos							firstStart;		// of PCDATA nodes
		Types.NodePos							lastStop;			// of PCDATA nodes
		Types.Ordinal							firstOrd;			// first preorder ordinal in subtree
		Types.Ordinal							lastOrd;			// last preoder ordinal in subtree		
	END;
	EXPORT PhrasePosting := RECORD
		Types.WordType						typWord1;
		Types.Nominal							nominal1;
		Types.WordType						typWord2;
		Types.Nominal							nominal2;
		Types.TermLength					spaces;		// white space is between these 2
		Types.DocID								docID;
		Types.KWP 								kwpBegin;
		Types.KWP 								kwpEnd;
		Types.NodePos							start;
		Types.NodePos 						stop;
		Types.PathNominal					path;		// cononical path
		Types.Ordinal							preorder;
		Types.Ordinal							parentOrd;
		Types.Depth								depth1;	
		Types.Depth								depth2;	
		Types.LetterPattern				lp1;
		Types.LetterPattern				lp2;
		Types.rci									mcsi;
		Types.rci									pcsi;
		Types.TermString					term1{MAXLENGTH(Types.MaxTermLen)};
		Types.TermString					term2{MAXLENGTH(Types.MaxTermLen)};
	END;
	EXPORT PathPosting := RECORD
		Types.Depth       pathLen;					// Depth of terminal node
		Types.Depth				pos;							// ordinal position in path
		Types.PathNominal path;							// Nominal for Path
		Types.NodeType    typXML;						// Type of terminal node
		Types.TagNominal	nominal;					// Nominal of Element or Attribute
	END;
	// Dictionary, combined terms and XML items
	EXPORT DictionaryEntry := RECORD
		Types.TermString				term{MAXLENGTH(Types.MaxTermLen)};
		Types.WordType					typ;
		Types.Nominal						nominal;
	END;
 // Dictionary layout for building index.
	export DictIndex := {
		Types.TermString  term {maxlength(Types.MaxTermLen)},
		Types.WordType    typ,
		Types.DictNominal nominal,
		Types.TermFixed   trm20,
		Types.DictHash    term_hash,
	};
	// Freq table for weights if necessary
	EXPORT FreqTab := RECORD
		Types.WordType					typ;
		Types.Nominal						nominal;
		Types.Frequency					docFreq;
		Types.Frequency					termFreq;
	END;
	
	// Search Request processing
	EXPORT OperationAttributes := RECORD
		Types.OpCode			op;
		UNSIGNED1					opRank;
		UNSIGNED1					defaultInputStreams;
		BOOLEAN						naryMerge;
		Types.Ops_Mask		inputMask;
		Types.Ops_Mask		rsltType;
		Types.Ops_Source	sourceSel;
	END;
	EXPORT Rqst_String := RECORD
		UNICODE	rqst{MAXLENGTH(Constants.Max_Rqst_Length)};
	END;
	EXPORT Oprnd := RECORD
		Types.Stage					stageIn;		
		Types.DeltaKWP			leftWindow;		// 0 if this is first term
		Types.DeltaKWP			rightWindow;	// 0 if this is first term
		BOOLEAN							suppress;
	END;
	EXPORT GetOperand := RECORD
		Types.TermString		srchArg{MAXLENGTH(Types.MaxTermlen)};
		Types.TermString		s1Arg{MAXLENGTH(Types.MaxTermLen)};
		Types.TermString		s2Arg{MAXLENGTH(Types.MaxTermLen)};
		UNSIGNED4						n1Arg;
		UNSIGNED4						n2Arg;
		Types.TermID				id;
		Types.NodeType			typXML;
		Types.WordType			typWord;
		BOOLEAN							suppress;
		BOOLEAN							chkParent;
		BOOLEAN							wsBetween;
		Types.NominalSet		nominals{MAXCOUNT(Constants.Max_Wild)};
		Types.NominalSet		parents{MAXCOUNT(2)};		// the parent, or ALL, or empty
		Types.PathSet				paths{MAXCOUNT(Constants.Max_Path_Nominals)};
		Types.NominalSet		tagNominals{MAXCOUNT(2)};	// the container, all or empty
		Types.NominalSet		next{MAXCOUNT(Constants.Max_Wild)};
	END;
	EXPORT Operation := RECORD
		Types.OpCode				op;
		Types.Stage					stage;
		GetOperand					getOprnd;	
		Types.OccurCount		cnt;				// Used in ATL with Get and with a Phrase
		Types.Ordinal				ordinal;
		DATASET(Oprnd)			inputs{MAXCOUNT(Constants.Max_Merge_Input)};
	END;
	EXPORT Message := RECORD
		UNSIGNED2						code{XPATH('code')};
		UNSIGNED2						start{XPATH('start_position')};
		UNSIGNED2						len{XPATH('length')};
		UNICODE							msg{XPATH('message'),MAXLENGTH(40)};
	END;
	
	// XPath support
	EXPORT NodeEntry := RECORD
		Types.NominalSet		nominalList{MAXCOUNT(2)};		//really just 1 or ALL
		BOOLEAN							float;
	END;
	EXPORT Path_Query := RECORD
		Types.Ordinal				ref;
		Types.NodeType			typTarget;
		DATASET(NodeEntry)	nodes{MAXCOUNT(Constants.Max_Node_Depth)};
	END;
	EXPORT Path_Answer := RECORD
		Types.Ordinal				ref;
		Types.PathSet				pathSet{MAXCOUNT(Constants.Max_Path_Nominals)};
	END;
	EXPORT XML_Filter := RECORD
		Types.RqstOffset		getPosition;
		Types.RqstOffset		fltPosition;
		UNICODE							tagName{MAXLENGTH(Constants.Max_Token_Length)};
		Types.NodeType			typXML;			// Element => [Element, Singleton]
		Types.WordType			typWord;
		Types.PathSet				pathSet{MAXCOUNT(Constants.Max_Path_Nominals)};
		Types.NominalSet		tagNominals{MAXCOUNT(2)};	// really just 1 or ALL
		Types.NominalSet		parents{MAXCOUNT(2)};
		BOOLEAN							isConnector;
		BOOLEAN							suppress;
		BOOLEAN							chkParent;
	END;
	
	// Saerch answers and intermediates
	EXPORT HitRecord := RECORD
		Types.KWP 								kwpBegin;
		Types.KWP 								kwpEnd;
		Types.NodePos							start;
		Types.NodePos 						stop;
		Types.TermID							termID;
	END;
	EXPORT DocHit := RECORD
		Types.DocID								docID;
		HitRecord;
	END;
	EXPORT MergeWork := RECORD(DocHit)
		Types.Section							sect;
		Types.WordType						typLast;
	END;
	EXPORT MergeWorkList := RECORD
		Types.DocID								docID;
		Types.KWP 								kwpBegin;
		Types.NodePos							start;
		Types.KWP 								kwpEnd;
		Types.NodePos 						stop;
		Types.Section							sect;
		Types.TermID							termID;
		Types.Ordinal							ord;
		Types.Ordinal							filter;
		Types.Ordinal							preorderL;		// position in tree of left side
		Types.Ordinal							preorderR;		// posiion in tree of right side
		Types.Ordinal							parentOrd;		// parent position
		Types.Ordinal							firstOrd;			// first preorder ordinal in subtree
		Types.Ordinal							lastOrd;			// last preoder ordinal in subtree
		BOOLEAN										incrKWP;			// this increments keyword position
		BOOLEAN										chkPos;				// check adjacent start/stop values
		BOOLEAN										chkParent;
		DATASET(HitRecord)				hits{MAXCOUNT(Constants.Max_Merge_Input)};
	END;
	EXPORT MWGroup := RECORD
		Types.DocID								docID;
		Types.Section							sect;
		DATASET(HitRecord)				hits{MAXCOUNT(Constants.Max_DocHits)};
	END;
	EXPORT AnswerRecord := RECORD
		Types.DocID								docID;
		DATASET(HitRecord)				hits{MAXCOUNT(Constants.Max_DocHits)};
	END;
	
	// Service output
	EXPORT TermDisplay := RECORD
		Types.TermID					id;
		Types.TermString			srchArg{MAXLENGTH(types.maxTermlen), XPATH('word')};
		Types.TermString			s1Arg{MAXLENGTH(types.maxTermlen), XPATH('attr1')};
		Types.TermString			s2Arg{MAXLENGTH(types.maxTermlen), XPATH('attr2')};
		STRING								typXML{MAXLENGTH(25), XPATH('Type_XML')};
		STRING								typWord{MAXLENGTH(25), XPATH('Type_Term')};
	END;
	EXPORT StageDisplay	:= RECORD
		Types.Stage						stageIn{XPATH('Stage')};
		Types.DeltaKWP				leftWindow{XPATH('Left_Window')};
		Types.DeltaKWP				rightWindow{XPATH('Right_Window')};
	END;
	EXPORT OperationDisplay := RECORD
		Types.OpCode					op;
		STRING								opName{MAXLENGTH(25)};
		Types.Stage						stage;
		BOOLEAN								termInput;
		TermDisplay 					term;
		Types.Ordinal					ordinal{XPATH('Ordinal_Position')};
		Types.OccurCount			minOccurs{XPATH('Min_Occurrences')};
		DATASET(StageDisplay)	inputs{MAXCOUNT(Constants.Max_Merge_Input),
																 XPATH('Inputs/Input')};
	END;
	EXPORT HitDisplay := RECORD
		Types.NodePos							start{XPATH('hit-start')};
		Types.NodePos 						stop{XPATH('hit-stop')};
		Types.TermID							termID{XPATH('term-id')};
	END;
	EXPORT Property := RECORD
		UNICODE 		property_name{MAXLENGTH(Constants.Max_Prop_Name),
															XPATH('property-name')};
		UNICODE			property_value{MAXLENGTH(Constants.Max_Prop_Value),
															 XPATH('property-value')};
	END;
	EXPORT Doc := RECORD
		Types.lni									lni{XPATH('lni')};
		Types.DocLength						size{XPATH('size')};
		Types.DocVersion					version{XPATH('version')};
		Types.rci									rci{XPATH('rci')};
		Types.Depth								level{XPATH('chunk-level')};
		DATASET(Property)					props{MAXCOUNT(64),XPATH('props/property')};
		DATASET(HitDisplay)				hits{MAXCOUNT(Constants.Max_DocHits),
																	 XPATH('hits/hit')};
	END;
	EXPORT SearchRequest := RECORD
		UNICODE										srchRqst{MAXLENGTH(10000),
																			 XPATH('input-search')};
		DATASET(OperationDisplay)	srchOps{MAXCOUNT(Constants.Max_Ops), 
																			XPATH('Operations/Operation')};
		DATASET(Message)					errors{MAXCOUNT(Constants.Max_Ops),
																			XPATH('Errors/Message')};
		DATASET(Message)					warnings{MAXCOUNT(Constants.Max_Ops),
																			XPATH('Warnings/Message')};
	END;
	EXPORT SearchResults := RECORD
		UNSIGNED4									doc_count{XPATH('doc-count')};
		DATASET(Doc)							docs{MAXCOUNT(Constants.Max_Docs_Complex),XPATH('docs/doc')};
		SearchRequest							request{XPATH('search-request')};
	END;

	export Work1 := record(Doc - hits)
		AnswerRecord.docID;
		AnswerREcord.hits;
		XCR_Storage.Types.Hierarchy  hierarchy;
		XCR_Storage.Types.SortKey    sort_key;
	end;
			
END;
...
EXPORT Merge_V2(XCR_Common.Options.Model info, Layouts.Operation sr,
	...
	// sets
	Indx 					:= Keys(info).InversionX2;
	IndxElement		:= Keys(info).ElementX;
	IndxAttr			:= Keys(info).AttributeX;
	IndxAttrRng		:= Keys(info).AttrRangeX;
	IndxNumeric		:= Keys(info).NumericAttx;
	IndxKPhrase		:= Keys(info).KPhrasex;
	IndxPhrase		:= Keys(info).Phrasex;
	SourceX				:= Keys(info).SourceX;
	EmptyHitSet		:= DATASET([], HitRecord);
 	
	// Helpers for nominal tests	
	OprndTypWord	:= sr.getOprnd.typWord;
	OprndTypXML		:= sr.getOprnd.typXML;
	...
	KeyWordTypes	:= [Types.WordType.Text, 		Types.WordType.Date,
										Types.WordType.Number, 	Types.WordType.Symbol];
	XMLTagTypes		:= [Types.NodeType.Element, Types.NodeType.Singleton,
										Types.NodeType.Attribute];
	...
	wildMatch(UNICODE t) := UNICODELIB.UnicodeWildMatch(t, OprndSrchArg, FALSE);
	...
	HitRecord makeHit(Types.KWP kwpBegin, Types.KWP kwpEnd, 
										Types.NodePos start, Types.NodePos stop) := TRANSFORM
		...
		SELF.stop				:= stop;
	END;
	MergeWorkList copyAIS(Types.DocID docID, Types.WordType typWord,
												Types.KWP kwpBegin, Types.KWP kwpEnd, 
												Types.NodePos start, Types.NodePos stop,
		...
		SELF.sect				:= sr.stage;
		SELF.ord				:= 0;
		SELF.incrKWP		:= typWord IN KeyWordTypes;
		SELF.termID			:= OprndTermID;
		SELF.filter 		:= 0;
	...
	// ***************************************************************************
	// RCI get
	rci0	:= SourceX(KEYED(mcsi IN OprndNominals));
	rci1	:= STEPPED(rci0, docID, kwpBegin, start, kwpEnd); //, stop);  compiler err
	rci2	:= PROJECT(rci1, copyAIS(LEFT.docID, Types.WordType.Meta,
																 LEFT.kwpBegin, LEFT.kwpEnd, LEFT.start, LEFT.stop, 
																 0, 0, 0, 0));
	Proj_RCIGET := SORTED(rci2, docID, kwpBegin, start, kwpEnd, stop);
	
	// No-op get for suppressed reads
	noop0 := Indx(KEYED(typWord=0 AND nominal=Constants.Nominal_Noone));
	noop1 := STEPPED(noop0, docID, kwpBegin, start, kwpEnd, stop);
	noop2 := PROJECT(noop1, copyAIS(LEFT.docID, LEFT.typWord, LEFT.kwpBegin,
														LEFT.kwpEnd, LEFT.start, LEFT.stop,
														LEFT.preorder, LEFT.parentOrd, 
														LEFT.preorder, LEFT.preorder));
	Proj_NOGET := SORTED(noop2, docID, kwpBegin, start, kwpEnd, stop);
	
	// No-op merge for merges with all but one input suppressed
	NoMerge := SORTED(inputs[1], docID, filter, kwpBegin, start, kwpEnd, stop);
	
	// Get operation
	p0 := Indx(KEYED(typWord=OprndTypWord AND nominal IN OprndNominals)
							AND path IN PathNominals AND typXML=OprndTypXML);
	p1 := STEPPED(p0, docID, kwpBegin, start, kwpEnd, stop);
	p2 := PROJECT(p1, copyAIS(LEFT.docID, LEFT.typWord, LEFT.kwpBegin,
														LEFT.kwpEnd, LEFT.start, LEFT.stop,
														LEFT.preorder, LEFT.parentOrd, 
														LEFT.preorder, LEFT.preorder));
	Proj_GET := SORTED(p2, docID, kwpBegin, start, kwpEnd, stop);
	
	// Verbatim filtering get operation
	q0 := Indx(KEYED(typWord=OprndTypWord AND nominal IN OprndNominals)
						 AND path IN PathNominals AND typXML=OprndTypXML
						 AND IF(hasWild, wildMatch(term), term=OprndSrchArg));
	q1 := STEPPED(q0, docID, kwpBegin, start, kwpEnd, stop);
	q2 := PROJECT(q1, copyAIS(LEFT.docID, LEFT.typWord, LEFT.kwpBegin,
														LEFT.kwpEnd, LEFT.start, LEFT.stop,
														LEFT.preorder, LEFT.parentOrd, 
														LEFT.preorder, LEFT.preorder));
	Proj_LITGET := SORTED(q2, docID, kwpBegin, start, kwpEnd, stop);
	
	// Get tag operation
	s0 := Indx(KEYED(typWord=OprndTypWord AND nominal IN OprndNominals)
						 AND parent IN OprndParents AND path IN pathNominals 
						 AND typXML IN XMLTagTypes);
	s1 := STEPPED(s0, docID, kwpBegin, start, kwpEnd, stop);
	s2 := PROJECT(s1, copyAIS(LEFT.docID, LEFT.typWord, LEFT.kwpBegin,
														LEFT.kwpEnd, LEFT.start, LEFT.stop,
														LEFT.preorder, LEFT.parentOrd, 
														LEFT.firstOrd, LEFT.lastOrd));
	Proj_TAGGET := SORTED(s2, docID, kwpBegin, start, kwpEnd, stop);
	
	// Keyword Phrase Get
	kph0 := IndxKPhrase(KEYED(nominal1 IN OprndNominals AND nominal2 IN NextNominals)
											AND path IN PathNominals);
	kph1	:= STEPPED(kph0, docID, kwpBegin, start, kwpEnd, stop);
	kph2	:= PROJECT(kph1, copyAIS(LEFT.docID, OprndTypWord, LEFT.kwpBegin,
																 LEFT.kwpEnd, LEFT.start, LEFT.stop,
														LEFT.preorder, LEFT.parentOrd, 
														LEFT.preorder, LEFT.preorder));
	Proj_GETKPH := SORTED(kph2, docID, kwpBegin, start, kwpEnd, stop);
	
	// Keyword Literal Phrase get
	lkph0 := IndxKPhrase(KEYED(nominal1 IN OprndNominals AND nominal2 IN NextNominals)
											AND path IN PathNominals
											AND IF(hasWild, wildMatch(term1), term1=OprndSrchArg));
	lkph1 := STEPPED(lkph0, docID, kwpBegin, start, kwpEnd, stop);
	lkph2 := PROJECT(lkph1, copyAIS(LEFT.docID, OprndTypWord, LEFT.kwpBegin,
																 LEFT.kwpEnd, LEFT.start, LEFT.stop,
																 LEFT.preorder, LEFT.parentOrd, 
																 LEFT.preorder, LEFT.preorder));
	Proj_LITKPH := SORTED(lkph2, docID, kwpBegin, start, kwpEnd, stop);
	
	// Phrase Get
	ph0		:= IndxPhrase((KEYED(nominal1 IN OprndNominals AND nominal2 IN NextNominals)
											AND path IN PathNominals));
	ph1		:= STEPPED(ph0, docID, kwpBegin, start, kwpEnd, stop);
	ph2		:= PROJECT(ph1, copyAIS(LEFT.docID, OprndTypWord, LEFT.kwpBegin,
																 LEFT.kwpEnd, LEFT.start, LEFT.stop,
																 LEFT.preorder, LEFT.parentOrd, 
																 LEFT.preorder, LEFT.preorder));
	Proj_GETPH := SORTED(ph2, docID, kwpBegin, start, kwpEnd, stop);
	
	// Literal Phrase Get
	lph0 	:= IndxPhrase(KEYED(nominal1 IN OprndNominals AND nominal2 IN NextNominals)
											AND path IN PathNominals
											AND IF(hasWild, wildMatch(term1), term1=OprndSrchArg));
	lph1 := STEPPED(lph0, docID, kwpBegin, start, kwpEnd, stop);
	lph2 := PROJECT(lph1, copyAIS(LEFT.docID, OprndTypWord, LEFT.kwpBegin,
																 LEFT.kwpEnd, LEFT.start, LEFT.stop,
																 LEFT.preorder, LEFT.parentOrd, 
																 LEFT.preorder, LEFT.preorder));
	Proj_LITPH := SORTED(lph2, docID, kwpBegin, start, kwpEnd, stop);

	// Match Whitespace phrase
	mph0 	:= IndxPhrase(KEYED(nominal1 IN OprndNominals AND nominal2 IN NextNominals)
											AND path IN PathNominals
											AND (  (wsBetween AND spaces > 0)
													OR (NOT wsBetween AND spaces = 0))
											AND IF(hasWild, wildMatch(term1), term1=OprndSrchArg));
	mph1 := STEPPED(mph0, docID, kwpBegin, start, kwpEnd);
	mph2 := PROJECT(mph1, copyAIS(LEFT.docID, OprndTypWord, LEFT.kwpBegin,
																 LEFT.kwpEnd, LEFT.start, LEFT.stop + LEFT.spaces,
																 LEFT.preorder, LEFT.parentOrd, 
																 LEFT.preorder, LEFT.preorder));
	Proj_MWSPH := SORTED(mph2, docID, kwpBegin, start, kwpEnd, stop);
	
	// Element Span get
	pcd0 := IndxElement(KEYED(nominal IN OprndNominals) 
											AND path IN PathNominals AND parent IN OprndParents
											AND NOT empty);
	pcd1 := STEPPED(pcd0, docID, kwpBegin, start, kwpEnd, stop);
	pcd2 := PROJECT(pcd1, copyAIS(LEFT.docID, OprndTypWord, LEFT.kwpBegin, 
																LEFT.kwpEnd, LEFT.start, LEFT.stop,
																 LEFT.preorder, LEFT.parentOrd, 
																 LEFT.firstOrd, LEFT.lastOrd));
	Proj_PCD := SORTED(pcd2, docID, kwpBegin, start, kwpEnd, stop);
	
	// Element empty span get
	emp0 := IndxElement(KEYED(nominal IN OprndNominals)
											AND path IN PathNominals AND parent IN OprndParents
											AND empty);
	emp1 := STEPPED(emp0, docID, kwpBegin, start, kwpEnd, stop);
	emp2 := PROJECT(emp1, copyAIS(LEFT.docID, OprndTypWord, LEFT.kwpBegin, 
																LEFT.kwpEnd, LEFT.start, LEFT.stop,
																LEFT.preorder, LEFT.parentOrd, 
																LEFT.firstOrd, LEFT.lastOrd));
	Proj_EMP := SORTED(emp2, docID, kwpBegin, start, kwpEnd, stop);

	// AND operation
	and_0 := MERGEJOIN(inputs, 
						 STEPPED(LEFT.docID=RIGHT.docID),
						 docID, kwpBegin, start, kwpEnd, stop, termID);
	and_1	:= PROJECT(and_0, updateSource(LEFT));
	AND_Stages := SORTED(and_1, docID, kwpBegin, start, kwpEnd, stop, termID);
	
	// Filtered AND operation
	fand_0 := MERGEJOIN(inputs, 
						 STEPPED(LEFT.docID=RIGHT.docID) AND LEFT.filter=RIGHT.filter,
						 docID, kwpBegin, start, kwpEnd, stop, termID);
	fand_1	:= PROJECT(fand_0, updateSource(LEFT));
	FAND_Stages := SORTED(fand_1, docID, kwpBegin, start, kwpEnd, stop, termID);
	
	// And NOT operation
	andnot_0 := MERGEJOIN(inputs, 
						STEPPED(LEFT.docID=RIGHT.docID),
						docID, kwpBegin, start, kwpEnd, stop, termID, LEFT ONLY);
	andnot_1 := PROJECT(andnot_0, updateSource(LEFT));
	ANDNOT_Stages := SORTED(andnot_1, docID, kwpBegin, start, kwpEnd, stop);
	...
	FANDNT_Stages := SORTED(fandnt_1, docID, kwpBegin, start, kwpEnd, stop);
	...
	phrase_1 := JOIN(inputs, 
						...
						fuseAIS(LEFT,ROWS(LEFT)), SORTED(docID, kwpBegin));
	Phrase_Stages := SORTED(phrase_1, docID, kwpBegin, start, kwpEnd, stop);
	
	// BUT NOT operation
	butnot_0 := MERGEJOIN(inputs, 
									STEPPED(LEFT.docID=RIGHT.docID)
									AND LEFT.start BETWEEN RIGHT.start AND RIGHT.stop,
									SORTED(docID, kwpBegin, start, kwpEnd, stop), LEFT ONLY);
	butnot_1	:= PROJECT(butnot_0, updateSource(LEFT));
	BUTNOT_Stages := SORTED(butnot_1, docID, kwpBegin, start, kwpEnd, stop);
	
	// OR operation
	or_0 := MERGE(inputs, docID, kwpBegin, start, kwpEnd, stop, termID,
								SORTED(docID, kwpBegin, start, kwpEnd, stop, termID));
	or_1 := PROJECT(or_0, updateSource(LEFT));
	OR_Stages := SORTED(or_1, docID, kwpBegin, start, kwpEnd, stop);
	
	// Cardinality Operation
	crd_test(c) := CASE(sr.op, code_ATL=> c>=sr.cnt, code_ATX=> c=sr.cnt, c<=sr.cnt);
	crd_1	:= GROUP(SORTED(inputs[1], docID, kwpBegin, start, kwpEnd, stop), docID);
	crd_2	:= HAVING(crd_1, crd_test(COUNT(ROWS(LEFT))));
	crd_3	:= UNGROUP(crd_2);
	crd_4	:= PROJECT(crd_3, updateSource(LEFT));
	Card_Stages := SORTED(crd_4, docID, kwpBegin, start, kwpEnd, stop);
	
	// Filtered Cardinality operations
	fcrd_test(c):=CASE(sr.op,code_FLT_ATL=>c>=sr.cnt,code_FLT_ATM=>c<=sr.cnt,c=sr.cnt);
	fcrd_0 := SORTED(inputs[1], docID, filter, kwpBegin, start, kwpEnd, stop);
	fcrd_1 := GROUP(fcrd_0, docID, filter);
	fcrd_2 := HAVING(fcrd_1, fcrd_test(COUNT(ROWS(LEFT))));
	fcrd_3 := UNGROUP(fcrd_2);
	fcrd_4 := PROJECT(fcrd_3, updateSource(LEFT));
	F_Card_Stages := SORTED(fcrd_4, docID, kwpBegin, start, kwpEnd, stop);
	...
	MergeWorkList proxMergeAIS(MergeWorkList l, DATASET(MergeWorkList) rws) := TRANSFORM
		...
		SELF.docID		:= IF(noOverlap, l.docID, SKIP);
		SELF.termID		:= l.termID;
		SELF.sect			:= sr.stage;
	...
	proxTest(MergeWorkList l, MergeWorkList r) 
				...
				OR l.kwpBegin BETWEEN r.kwpBegin-leftWindow(r.sect)
													AND r.kwpEnd+rightWindow(r.sect);
	prox_1:= JOIN(inputs, 
								STEPPED(LEFT.docID=RIGHT.docID) AND proxTest(LEFT,RIGHT),
								proxMergeAIS(LEFT, ROWS(LEFT)),
								SORTED(docID, kwpBegin, start, kwpEnd, stop));
	Prox_Stages := SORTED(prox_1, docID, kwpBegin, start, kwpEnd, stop);
	...
	f_prox_1 := JOIN(inputs,
								...
								proxMergeAIS(LEFT, ROWS(LEFT)),
								SORTED(docID, kwpBegin, start, kwpEnd, stop));
	F_Prox_Stages := SORTED(f_prox_1, docID, kwpBegin, start, kwpEnd, stop);
	
	// NOT W/n operation
	notw_0	:= MERGEJOIN(inputs, 
										STEPPED(LEFT.docID=RIGHT.docID) 
										AND proxtest(LEFT,RIGHT),
										docID, kwpBegin, start, kwpEnd, stop, LEFT ONLY);
	notw_1	:= PROJECT(notw_0, updateSource(LEFT));
	NOTW_Stages := SORTED(notw_1, docID, kwpBegin, start, kwpEnd, stop);
	...
	F_NOTW_Stages := SORTED(f_notw_1, docID, kwpBegin, start, kwpEnd, stop);
	...
	path_0	:= JOIN(inputs, 
									...
									SORTED(docID, kwpBegin, start, kwpEnd, stop));
	PATH_Stages := SORTED(path_0, docID, kwpBegin, start, kwpEnd, stop);
	...
	MergeWorkList filterAIS(MergeWorkList flt, MergeWorkList trms) := TRANSFORM
		...
		SELF.docID		:= IF(allConjuncts, trms.docID, SKIP);
	...
	cntr_0	:= JOIN(inputs,
									...
									SORTED(docID, kwpBegin, start, kwpEnd, stop));
	CNTR_Stages := SORTED(cntr_0, docID, kwpBegin, start, kwpEnd, stop);
	...
	MergeWorkList filterPath(MergeWorkList path, MergeWorkList trms) := TRANSFORM
		...
		allConjuncts	:= NOT EXISTS(failedInclList);
		SELF.docID		:= IF(allConjuncts, path.docID, SKIP);
		SELF.sect			:= sr.stage;
		SELF					:= path;
	END;
	pred_0 	:= JOIN(inputs,
									...
									SORTED(docID, kwpBegin, start, kwpEnd, stop));
	PRED_Stages := SORTED(pred_0, docID, kwpBegin, start, kwpEnd, stop);
	...
	xpred_0 := JOIN(inputs,
									...
									SORTED(docID, kwpBegin, start, kwpEnd, stop));
	XPRED_Stages := SORTED(xpred_0, docID, kwpBegin, start, kwpEnd, stop);
	
	// Range match for Attribute values
	u0 := IndxAttrRng(KEYED(attrNominal IN OprndNominals AND parent IN OprndParents)
										AND fd(val10) AND gd(value));
	u3 := STEPPED(u0, docID, kwpBegin, start, kwpEnd, stop);
	u4 := PROJECT(u3, copyAIS(LEFT.docID,Types.WordType.AttrVal,LEFT.kwpBegin,
														LEFT.kwpEnd,LEFT.start,LEFT.stop,
														LEFT.preorder, LEFT.parentOrd,
														LEFT.preorder, LEFT.preorder));
	Proj_ATRGGET := SORTED(u4, docID, kwpBegin, start, kwpEnd, stop);
	
	// Get match to attribute value
	v0 := IndxAttr(KEYED(attrNominal IN OprndNominals AND fd(val10) 
											 AND parent IN OprndParents)
								 AND gd(value));
	v3 := STEPPED(v0, docID, kwpBegin, start, kwpEnd, stop);
	v4 := PROJECT(v3, copyAIS(LEFT.docID,Types.WordType.AttrVal,LEFT.kwpBegin,
														LEFT.kwpEnd,LEFT.start,LEFT.stop,
														LEFT.preorder, LEFT.parentOrd,
														LEFT.preorder, LEFT.preorder));
	Proj_ATTRGET := SORTED(v4, docID, kwpBegin, start, kwpEnd, stop);
	...
	MergeWorkList enumerateHits(MergeWorkList lr, MergeWorkList rr) := TRANSFORM
		...
		SELF := rr;
	END;
	ord_1  := GROUP(SORTED(inputs[1], docID, kwpBegin, start, kwpEnd, stop), docID);
	ord_2	 := ITERATE(ord_1, enumerateHits(LEFT, RIGHT));
	ord_3	 := ord_2(ord=sr.ordinal);
	ord_4	 := UNGROUP(ord_3);
	Ord_Stages := SORTED(ord_4, docID, kwpBegin, start, kwpEnd, stop);
	...
	MergeWorkList getFilterValue(MergeWorkList term, MergeWorkList flt) := TRANSFORM
		...
		SELF := term;
	END;
	flt0 := JOIN(inputs, STEPPED(LEFT.docID=RIGHT.docID)
											 AND LEFT.start BETWEEN RIGHT.start AND RIGHT.stop,
							 getFilterValue(LEFT,RIGHT),
							 SORTED(docID, kwpBegin, start, kwpEnd, stop));
	SetFlt_Stages := SORTED(flt0, docID, kwpBegin, start, kwpEnd, stop);
	
	// Span equal 
	span0 := JOIN(inputs, STEPPED(LEFT.docID=RIGHT.docID)
								...
								SORTED(docID, kwpBegin, start, kwpEnd, stop));
	EMatch_Stages := SORTED(span0, docID, kwpBegin, start, kwpEnd, stop);
	
	// Keywords in span equal
	kspan0 := JOIN(inputs, STEPPED(LEFT.docID=RIGHT.docID)
												 AND LEFT.kwpBegin	= RIGHT.kwpBegin
												 AND LEFT.start 	 <=	RIGHT.start
												 AND LEFT.kwpEnd		= RIGHT.kwpEnd
												 AND LEFT.stop		 >= RIGHT.stop,
								 TRANSFORM(MergeWorkList, SELF.sect:=sr.stage, SELF:=LEFT),
								 SORTED(docID, kwpBegin, start, kwpEnd, stop));
	KMatch_Stages := SORTED(kspan0, docID, kwpBegin, start, kwpEnd, stop);
	
	// Document ID Filter
	dflt0	:= MERGEJOIN(inputs, STEPPED(LEFT.docID=RIGHT.docID),
								docID, kwpBegin, start, kwpEnd, stop, termID);
	dflt1	 := PROJECT(dflt0, updateSource(LEFT));
	DocFlt_Stages := SORTED(dflt1, docID, kwpBegin, start, kwpEnd, stop);
	//***************************************************************
	// Run merge
	mrgRslt := CASE(sr.op,
		code_GET 			=>	Proj_GET,

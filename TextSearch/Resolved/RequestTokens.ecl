...
EXPORT RequestTokens := MODULE
	...
	SHARED ExcludeTypeList := [Types.WordType.WhiteSpace, Types.WordType.Other];
	...
	SHARED NodeType			:= Types.NodeType;
	SHARED raw_Ordinary	:= RawTokenType.Ordinary;
	SHARED raw_Junk			:= RawTokenType.Junk;
	SHARED raw_SplitOp	:= RawTokenType.SplitOp;
	SHARED WordType			:= Types.WordType;
	SHARED StringToNCF	:= NumericCollationFormat.StringToNCF;
	...
	RawToken0 toToken(Layouts.Rqst_String rq) := TRANSFORM
		...
		SELF.ttOrg	 := SELF.tt;
		SELF.typWord := MAP(
								MATCHED(WordAllUpper) 					=> WordType.Text,
								MATCHED(WordAllLower) 					=> WordType.Text,
								MATCHED(WordMixedCase)					=> WordType.Text,
								MATCHED(WordNoLetters)					=> WordType.Text,
								MATCHED(SearchQuote)						=> WordType.Noise,
								MATCHED(SearchBSlash)						=> WordType.Noise,
								MATCHED(Noise4Search)						=> WordType.Noise,
								MATCHED(Symbol4Search)					=> WordType.Symbol,
								MATCHED(WhiteSpace)							=> WordType.WhiteSpace,
								MATCHED(AnyChar)								=> WordType.AnyChar,
								MATCHED(AnyPair)								=> WordType.AnyChar,
								MATCHED(FxElemName)							=> WordType.Element,
								MATCHED(FlElemName)							=> WordType.Element,
								MATCHED(AnyElement)							=> WordType.Element,
								MATCHED(FlAnyElement)						=> WordType.Element,
								MATCHED(PathAttribute)					=> WordType.Attribute,
								MATCHED(Attribute)							=> WordType.Attribute,
								MATCHED(AnyAttribute)						=> WordType.Attribute,
								MATCHED(AnyAttrCompare)					=> WordType.Attribute,
								MATCHED(AttrCompare)						=> WordType.Attribute,
								MATCHED(PathCompare)						=> WordType.Attribute,
								MATCHED(FloatAttrComp)					=> WordType.Attribute,
								MATCHED(FloatAttribute)					=> WordType.Attribute,
								MATCHED(Operator/Op_NOT)				=> WordType.Meta,
								MATCHED(Get_Special)						=> WordType.Element,
								MATCHED(PoundCodeWild)					=> WordType.Text,
		...
		SELF.s2Arg		:= MAP(
								...
								u'');
		SELF.typXML 	:= MAP(
								MATCHED(FxElemName)							=> NodeType.Element,
								MATCHED(FlElemName)							=> NodeType.Element,
								MATCHED(AnyElement)							=> NodeType.Element,
								MATCHED(FlAnyElement)						=> NodeType.Element,
								MATCHED(PathAttribute)					=> NodeType.Attribute,
								MATCHED(FloatAttribute)					=> NodeType.Attribute,
								MATCHED(Attribute)							=> NodeType.Attribute,
								MATCHED(AnyAttribute)						=> NodeType.Attribute,
								MATCHED(FloatAttrComp)					=> NodeType.Attribute,
								MATCHED(AttrCompare)						=> NodeType.Attribute,
								MATCHED(PathCompare)						=> NodeType.Attribute,
								MATCHED(Op_Not)									=> NodeType.UNKNOWN,
								MATCHED(Get_Special)						=> NodeType.ELement,
								NodeType.PCDATA);
	...
	EXPORT MarkedTokens(UNICODE qstr) := FUNCTION
		RawToken0 lrPass(RawToken0 prev, RawToken0 this) := TRANSFORM
			...
			SELF.wildCard	:= MAP(
				...
				this.typWord=WordType.WhiteSpace				=> FALSE,
				this.typWord=WordType.Noise							=> FALSE,
				this.typWord=WordType.Symbol						=> FALSE,
				this.typWord=WordType.AnyChar						=> FALSE,
	...
	EXPORT AllTokens(UNICODE qstr) := FUNCTION
		...
		RawToken0 rollSameOrdinal(RawToken0 l, RawToken0 r) := TRANSFORM
			...
			SELF.typXML		:= l.typXML;
			SELF.typWord	:= WordType.Text;
		...
		RawToken0 splitToken(RawToken0 tok) := TRANSFORM
			...
			SELF.typWord	:= MAP(
								...
								MATCHED(WordMixedCase)					=> WordType.Text,
								MATCHED(WordNoLetters)					=> WordType.Text,
								MATCHED(Noise)									=> WordType.Noise,
								MATCHED(Symbol)									=> WordType.Symbol,
								MATCHED(WhiteSpace)							=> WordType.WhiteSpace,
								MATCHED(AnyChar)								=> WordType.AnyChar,
								MATCHED(AnyPair)								=> WordType.AnyChar,
								Types.WordType.Other);
			SELF.typXML		:= tok.typXML;
			SELF.op				:= MAP(
								tok.typXML = NodeType.Attribute	=>	code_MWSGET,
								MATCHED(WordAllUpper)						=>	code_LITGET,
								MATCHED(WordMixedcase)					=>	code_LITGET,
								code_GET);
	...
	RawToken0 adjTypes(RawToken0 lr) := TRANSFORM
		SELF.tt				:= MAP(
							  ...
							  lr.tt);
		SELF.typWord	:= MAP(
								NOT lr.quoted										=> lr.typWord,
								isLP(lr.tt)											=> WordType.Symbol,
								isRP(lr.tt)											=> WordType.Symbol,
								lr.tt=RawTokenType.LSQB					=> WordType.Noise,
								lr.tt=RawTokenType.RSQB					=> WordType.Noise,
								isWildCard(lr.tt)								=> WordType.Noise,
								lr.typWord);
		SELF.op				:= MAP(
								NOT lr.quoted										=> lr.op,
							  ...
							  code_LITGET);
		SELF.typXML		:= IF(lr.quoted, NodeType.PCDATA, lr.typXML);
		SELF.msg			:= MAP(
	...
	EXPORT RqstTokens(UNICODE qstr) := FUNCTION
		...
		RawToken2 markDepth(RawToken2 prev, RawToken2 this) := TRANSFORM
			...
			SELF.attrExpr		:= MAP(
								...
								prev.pathExpr										=> FALSE,
								TRUE);
			SELF.typXML			:= IF(SELF.attrExpr, NodeType.Attribute, this.typXML);
			SELF.ordinal		:= prev.ordinal + 1;
